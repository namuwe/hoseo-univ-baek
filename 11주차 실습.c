...
//저장
void savefile(GOODS G[])
{
	FILE* fp;
	if (!(fp = fopen("GOODS_SAVE.bin", "wb"))) {
		printf("파일 쓰기중 오류가 발생하였습니다.\n");
		return 0;
	}
	fwrite(G, sizeof(GOODS), NUM, fp);
	printf("파일 쓰기가 완료되었습니다.\n");
	fclose(fp);
}

//불러오기
void loadfile(GOODS G[])
{
	FILE* fp;
	if (!(fp = fopen("GOODS_SAVE.bin", "rb"))) {
		printf("파일 읽기중 오류가 발생하였습니다.\n");
		return 0;
	}
	fread(G, sizeof(GOODS), NUM, fp);
	printf("파일 읽기가 완료되었습니다.\n");
	fclose(fp);
}
